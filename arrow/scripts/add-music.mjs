 ...
 * scripts/make-music.py, with its whoosh placed at `release` seconds, so the
 * score always ends with the picture no matter how the timings are edited.
 */
...
if (!input || !output) {
  console.error('usage: node scripts/add-music.mjs <in.mp4> <out.mp4> [--release=7.75]');
  process.exit(1);
}
...
await run('python3', [path.join(root, 'scripts/make-music.py'), bed, `--length=${length.toFixed(2)}`, `--release=${args.release ?? 7.75}`]);
await run(ffmpeg, [
  '-y', '-i', input, '-i', bed,
  '-map', '0:v', '-map', '1:a',
