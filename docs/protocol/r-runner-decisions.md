...
| DEC-R-023 | All ordering that feeds outputs or fingerprints uses locale-independent (radix) sorting | `order()` and `sort()` differ between the C and C.UTF-8 collations, so row order, canonical-JSON key order and the dataset fingerprint would otherwise depend on the machine's locale and break replay. A test flips LC_COLLATE in-process and requires identical fingerprints and checksums. | Adopted |
